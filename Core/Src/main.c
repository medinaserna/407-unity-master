...
#define packetLength  39  //este nuevo tamaño es debido a mi nueva trama para Wayne Mayes
...
//uint8_t packetTxTime = 243; //mS  veneno
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  BSP_ACCELERO_Init();
//  HAL_ADC_Start_DMA(&hadc1, adcValues, sizeof(uint16_t)*3); //lo comento para probar  velocidades de Tx y funciono
  /* USER CODE END 2 */
...
//	  printf("X: %0.2f\tY: %0.2f\tZ: %0.2f\n", (float)XYZ[0], (float)XYZ[1], (float)XYZ[2]);
	...
	//  sprintf(packet + 34, "%0d", inputPort[3]); // para recortar la trama y se me ajuste al MAUI de Wayne
	 // sprintf(packet + 35, "%0d", inputPort[2]); // debo de recortar 4 bits
	 // sprintf(packet + 36, "%0d", inputPort[1]);
	 // sprintf(packet + 37, "%0d", inputPort[0]);

	  //Blue Button // this bit is hide to adjust packet (Sebastian)
